...
const PREGUNTAS_PILAR_3 = [
  "Indica qué tan identificado te sientes con las siguientes afirmaciones cotidianas. [Creo firmemente que mis logros dependen exclusivamente de mi esfuerzo diario y no de la buena suerte.]",
  "Indica qué tan identificado te sientes con las siguientes afirmaciones cotidianas. [Siento que la mayor parte de las cosas malas que me pasan son culpa de otras personas o de las circunstancias.]",
  "Indica qué tan identificado te sientes con las siguientes afirmaciones cotidianas. [Si fracaso en un objetivo, analizo qué hice mal para no repetirlo, en lugar de frustrarme con el entorno.]",
  "Indica qué tan identificado te sientes con las siguientes afirmaciones cotidianas. [A veces siento que no tiene sentido planificar demasiado porque el destino ya está escrito.]",
  "Indica qué tan identificado te sientes con las siguientes afirmaciones cotidianas. [Cuando trabajo en equipo y algo sale mal, asumo mi parte de la culpa en lugar de señalar a los demás de inmediato.]",
  "Indica qué tan identificado te sientes con las siguientes afirmaciones cotidianas. [Confío más en mi propia capacidad para resolver problemas inesperados que en esperar a que alguien más me ayude.]",
  "Indica qué tan identificado te sientes con las siguientes afirmaciones cotidianas. [Siento que el éxito de las personas a mi alrededor se debe más a sus contactos o suerte que a su talento.]",
];
...
export default function Pilar3() {
  const router = useRouter();
  const [respuestas, setRespuestas] = useState<Record<string, string>>({});
  const [error, setError] = useState("");

  useEffect(() => {
    const saved = sessionStorage.getItem("respuestas_pilar3");
  ...
  const handleContinuar = () => {
    const sinResponder = PREGUNTAS_PILAR_3.filter((p) => !respuestas[p]);
    if (sinResponder.length > 0) {
      setError(`Te faltan ${sinResponder.length} pregunta${sinResponder.length > 1 ? "s" : ""} por responder.`);
      return;
    }
    sessionStorage.setItem("respuestas_pilar3", JSON.stringify(respuestas));
    router.push("/evaluacion/pilar4");
  };

  const respondidas = Object.keys(respuestas).length;
  const progreso = (respondidas / PREGUNTAS_PILAR_3.length) * 100;
    ...
    <main className="min-h-screen bg-[var(--background)] flex justify-center px-4 py-12">
      <div className="w-full max-w-2xl">
        ...
        <div className="mb-6 bg-[var(--card)] border border-[var(--card-border)] rounded-xl px-5 py-3 shadow-sm">
          <div className="flex items-center justify-between text-xs text-[var(--muted-foreground)] mb-2">
            ...
            <span className="font-semibold text-[var(--foreground)]">
              {respondidas} / {PREGUNTAS_PILAR_3.length}
            </span>
          </div>
        ...
        <div className="space-y-4">
          {PREGUNTAS_PILAR_3.map((pregunta, idx) => (
            <div
              key={pregunta}
        ...
        <div className="mt-8 flex gap-3">
          <button
            onClick={() => router.push("/evaluacion/pilar2")}
