...
const PREGUNTAS_PILAR_4 = [
  "Indica qué tan identificado te sientes con las siguientes afirmaciones cotidianas. [Jamás he dicho una mentira, ni siquiera una pequeña, para salir de un apuro o evitar un problema.]",
  "Indica qué tan identificado te sientes con las siguientes afirmaciones cotidianas. [Nunca he sentido envidia o celos por el éxito de otra persona.]",
  "Indica qué tan identificado te sientes con las siguientes afirmaciones cotidianas. [Siempre estoy dispuesto a admitir mis errores de inmediato, sin importar las consecuencias.]",
  "Indica qué tan identificado te sientes con las siguientes afirmaciones cotidianas. [Jamás he perdido la paciencia, ni siquiera cuando estoy muy cansado o estresado.]",
  "Indica qué tan identificado te sientes con las siguientes afirmaciones cotidianas. [Nunca me he reído o sonreído ante un chiste de mal gusto o una situación inapropiada.]",
];
...
export default function Pilar4() {
  const router = useRouter();
  const [respuestas, setRespuestas] = useState<Record<string, string>>({});
  const [error, setError] = useState("");

  useEffect(() => {
    const saved = sessionStorage.getItem("respuestas_pilar4");
  ...
  const handleContinuar = () => {
    const sinResponder = PREGUNTAS_PILAR_4.filter((p) => !respuestas[p]);
    if (sinResponder.length > 0) {
      setError(`Te faltan ${sinResponder.length} pregunta${sinResponder.length > 1 ? "s" : ""} por responder.`);
      return;
    }
    sessionStorage.setItem("respuestas_pilar4", JSON.stringify(respuestas));
    router.push("/morosidad");
  };

  const respondidas = Object.keys(respuestas).length;
  const progreso = (respondidas / PREGUNTAS_PILAR_4.length) * 100;
      ...
      <div className="w-full max-w-2xl">
        <div className="text-center mb-8">
          ...
          <p className="text-[var(--muted-foreground)] text-sm">
            Último bloque. Indica qué tan identificado te sientes con las siguientes afirmaciones.
          </p>
        </div>
        ...
        <div className="mb-6 bg-[var(--card)] border border-[var(--card-border)] rounded-xl px-5 py-3 shadow-sm">
          <div className="flex items-center justify-between text-xs text-[var(--muted-foreground)] mb-2">
            ...
            <span className="font-semibold text-[var(--foreground)]">
              {respondidas} / {PREGUNTAS_PILAR_4.length}
            </span>
          </div>
        ...
        <div className="space-y-4">
          {PREGUNTAS_PILAR_4.map((pregunta, idx) => (
            <div
              key={pregunta}
        ...
        <div className="mt-8 flex gap-3">
          <button
            onClick={() => router.push("/evaluacion/pilar3")}
